...
import { alternarJefeCompletado, incrementarIntentos } from '../store';

const TarjetaJefe = ({ boss, onEdit, onDelete }) => {
  const dispatch = useDispatch();
  const { tema } = useTema();

  const handleToggleCompletion = () => {
    dispatch(alternarJefeCompletado(boss.id));
  };

  const handleIncrementAttempts = () => {
    dispatch(incrementarIntentos(boss.id));
  };
      ...
      <View style={styles.buttonContainer}>
        ...
        >
          <Ionicons 
            ...
            size={20} 
            color={boss.completed ? tema.success : tema.textMuted} 
          />
        ...
        >
          <Ionicons name="add-circle-outline" size={20} color={tema.warning} />
